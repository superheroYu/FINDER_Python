...
Pure-Python FINDER training infrastructure built with PyTorch, PyTorch Geometric, NetworkX, and Gymnasium. The project implements DQN-based graph optimization pipelines for Critical Node and Network Dismantling problems, including cost-aware variants, while keeping the codebase easy to run, inspect, and extend.
...
![FINDER overview](pictures/FINDER-Overview.jpg)
...
- Unified support for four variants: `cn`, `cn_cost`, `nd`, and `nd_cost`.
- Gymnasium-compatible environments backed by NetworkX graph states.
- Vectorized sampling through `SyncVectorEnv` / `AsyncVectorEnv`.
- PyTorch Geometric data interfaces that preserve real NetworkX node IDs.
- Config-driven experiments with JSON files plus command-line overrides.
- Optional modern DQN tricks: Double DQN, Dueling DQN, Huber loss, N-step replay, and prioritized replay.
- Built-in TensorBoard logging, checkpointing, best-model saving, deterministic seeding, and gradient clipping.
...
| --- | --- | --- |
| `cn` | Critical Node | Remove nodes to minimize connected-component decomposition score. |
...
All environments return dictionary observations containing a NetworkX graph and auxiliary features. Actions are real node labels in the current NetworkX graph, and `models/data_interfaces.py` keeps the mapping between PyG local indices and original node IDs.

## Installation

Use `uv` for environment creation and dependency installation. Python 3.12 is recommended.

```bash
uv venv --python 3.12

# CUDA build used by this repository's tested Windows setup:
...
# PyG compiled extensions matching torch 2.11.0 + CUDA 12.8.
uv pip install pyg_lib torch_scatter torch_sparse -f https://data.pyg.org/whl/torch-2.11.0+cu128.html
```

The CUDA wheel choice follows the official PyTorch install selector and PyG wheel matrix. On Linux with matching PyG wheels, you can replace `cu128` with another official CUDA wheel suffix such as `cu130`; keep the PyTorch wheel URL and PyG `data.pyg.org` URL on the same `torch + CUDA` combination.
...
On Linux + CUDA + `AsyncVectorEnv`, multiprocessing `spawn` must be configured before constructing `FinderVectorTrainer`. `train.py` handles this through `_setup_determinism_and_spawn(seed=...)`:
