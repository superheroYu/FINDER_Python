"""
Network Dismantling Cost Environment (Weighted)

This module implements the weighted Network Dismantling problem environment for the FINDER framework.
The goal is to select a set of nodes to remove from a graph to minimize the size of the largest connected component,
while accounting for node removal costs/weights.

Based on the original FINDER_ND_cost implementation.
"""
...
class NetworkDismantlingCostEnv(BaseFINDEREnv):
    """
    Network Dismantling Cost Environment (Weighted Version)

    In this environment, the agent selects nodes to remove from the graph, aiming to minimize the size of the Largest Connected Component (LCC),
    ...
    def __init__(
        ...
        training_type: str = 'degree'  # nd_cost defaults to degree weights
    ):
        """
        Initialize the Network Dismantling Cost environment.
    ...
    def _compute_reward(self, action: int) -> float:
        """
        Compute reward for the weighted network dismantling problem.

        The reward function considers both the impact on network connectivity
        (max connected component size) and the cost of removing nodes:

        reward = -(max CC size / total nodes) * (node weight / total weight)
        ...
        # Original FINDER ND cost reward formula
    ...
    def _get_info(self) -> Dict[str, Any]:
        ...
        # Add Network Dismantling Cost specific info
        info.update({
            'node_weights': self.node_weights.copy(),
            'total_node_weight': self.total_node_weight,
            'total_weight_removed': self.total_weight_removed,
            ...
            'problem_type': 'network_dismantling_cost',
            'weighted': True
        })
    ...
    def get_weight_efficiency(self) -> float:
        """
        Get current weight efficiency (CC reduction per unit weight).
    ...
    def render(self, mode: str = 'human', **kwargs):
        ...
        if mode == 'human':
            print(f"=== Network Dismantling Cost Environment ===")
            ...
            print(f"Dismantling ratio: {self.get_dismantling_ratio():.4f}")
    ...
    def _get_specific_render_info(self) -> List[str]:
        """Get ND_cost-specific rendering info."""
        info_lines = []
        ...
        info_lines.append(f"Problem type: Network Dismantling (weighted)")
