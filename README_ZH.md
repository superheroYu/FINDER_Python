...
<p>
  <b>基于 Python · PyTorch · PyTorch Geometric 的 <a href="https://www.nature.com/articles/s42256-020-0177-2">FINDER</a> 复现</b><br/>
  统一支持 Critical Node 与 Network Dismantling 两类图优化问题的 DQN 训练管线
</p>
...
<p>
  ...
  <b>中文</b> &nbsp;·&nbsp;
  <a href="https://www.nature.com/articles/s42256-020-0177-2">论文</a> &nbsp;·&nbsp;
  <a href="https://github.com/FFrankyy/FINDER">原始代码</a>
</p>

<img src="pictures/FINDER-Overview.jpg" alt="FINDER 方法概览" width="820"/>

</div>

---

> **一句话简介** —— 一套纯 Python、PyTorch 优先的 FINDER 复现实现。一份 CLI、一个 trainer 驱动 `cn`、`cn_cost`、`nd`、`nd_cost` 四个变体，运行在 Gymnasium 兼容的 NetworkX 环境之上，PyG 批数据保留真实节点 ID，配套 JSON 配置 + 命令行覆盖、TensorBoard、checkpoint 与现代 DQN 训练技巧，**无需 Cython 编译步骤**。
...
FINDER 关注复杂网络中的**关键节点选择问题**：找到一组节点，使得移除或激活它们后，网络连通性、鲁棒性或传播能力发生最大变化。这类问题可用于网络免疫、流行病控制、药物靶点发现、基础设施防护和传播营销等场景，但通常具有较高的组合优化复杂度。

我们使用 Python、PyTorch 和 PyTorch Geometric 对 FINDER 进行了完整重写，**围绕实际实验体验做了工程化增强**：

|                       | 特性 |
| ---                   | --- |
| **纯 Python 主路径**  | 环境、图处理、训练入口和配置系统都不需要 Cython 编译步骤。                  |
| **四个变体统一入口**  | `cn`、`cn_cost`、`nd`、`nd_cost` 共用同一套 CLI、trainer 与配置对象。     |
| **Gymnasium 兼容环境**| 单环境与向量环境都保留原始 NetworkX 图观测，便于调试与扩展。               |
| **真实节点 ID 映射**  | 从 NetworkX 到 PyG 的数据接口保留真实节点 ID，避免动作错位。              |
| **配置 + CLI 覆盖**   | `configs/` 中的 JSON 预设可随时通过命令行覆盖。                          |
| **现代 DQN 技巧可配置**| 支持 N-step replay、PER、Double DQN、Dueling DQN、Huber loss。           |
| **实验工程完整**      | TensorBoard 日志、checkpoint、best model、seed 与梯度裁剪都已集成。       |
...
> 所有环境统一返回包含 NetworkX 图与辅助特征的 **dict 观测**；动作语义是当前 NetworkX 图中的**真实节点标签**。[models/data_interfaces.py](models/data_interfaces.py) 会维护 PyG 局部索引与真实节点 ID 的映射，避免训练和环境交互时节点编号错位。

## 环境准备

本项目使用 [`uv`](https://docs.astral.sh/uv/) 创建环境和安装依赖。**建议使用 Python 3.12**。

```bash
# 1. 创建虚拟环境
uv venv --python 3.12

# 2. 安装 PyTorch（当前 Windows 测试环境使用 CUDA 12.8 wheel）
...
# 4. 安装与 torch 2.11.0 + CUDA 12.8 匹配的 PyG 编译扩展
uv pip install pyg_lib torch_scatter torch_sparse \
  -f https://data.pyg.org/whl/torch-2.11.0+cu128.html
```

> **Wheel 兼容性提示** —— CUDA wheel 的选择依据 PyTorch 官方安装选择器和 PyG wheel 矩阵。Linux 环境且 PyG 已提供对应 wheel 时，可以把 `cu128` 换成 `cu130` 等官方 CUDA 后缀；需要保持 PyTorch wheel URL 与 PyG `data.pyg.org` URL 的 `torch + CUDA` 组合一致。
...
<details>
<summary><b>常用命令行参数</b>（点击展开）</summary>
...
| 文件                              | 用途                                                  |
| ---                               | ---                                                   |
| `configs/cn_config.json`          | Critical Node 基线配置。                              |
| `configs/cn_cost_config.json`     | 带节点移除成本的 Critical Node。                      |
| `configs/nd_config.json`          | Network Dismantling 基线配置。                        |
| `configs/nd_cost_config.json`     | 带节点移除成本的 Network Dismantling。                |
| `configs/cn_full_config.json`     | 启用 Double + Dueling DQN、PER、Huber、N-step 的 CN。 |
| `configs/finder_defaults.json`    | 所有变体共享的默认字段。                              |

## 重要运行说明

> **Linux + CUDA + `AsyncVectorEnv` 场景** —— 需要在构造 `FinderVectorTrainer` 前设置 multiprocessing `spawn`。当前 [`train.py`](train.py) 已通过 `_setup_determinism_and_spawn(seed=...)` 统一处理：
>
...
如果本项目对你的研究有帮助，请**同时**引用本仓库实现和原始 FINDER 论文。

<details>
<summary><b>BibTeX —— 本仓库</b></summary>
...
<details>
<summary><b>BibTeX —— 原始 FINDER 论文</b></summary>
